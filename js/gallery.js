...
refs.imageList.innerHTML = imageCard;
...
const onImageClick = event => {
    ...
    const linkLargeImg = event.target.dataset.source;
    console.log(linkLargeImg); 

    const imageInfo = images.find(image => linkLargeImg === image.original);
    
    const basicLightBoxInstance = basicLightbox.create(`
        <div class="modal">
        <img class="larg-image" src="${imageInfo.original}" alt="${imageInfo.description}" />
        </div>
        `,
        {
            onShow: (instance) => {
...
refs.imageList.addEventListener('click', onImageClick);
